...
import { verifyProducts } from "./helper";
...
test.describe("Home Page", async () => {
  ...
  test("Product details and prices are shown in the page", async () => {
    await verifyProducts(page, "#product-1", "Black Heels", "45.00");
    await verifyProducts(page, "#product-2", "Bronze Sandals", "39.99");
    await verifyProducts(page, "#product-3", "Brown Shades", "20.00");
    await expect(page.locator("#product-3").locator(".sold-out").getByText("Sold Out")).toBeVisible();
    await verifyProducts(page, "#product-4", "Grey Jacket", "55.00");
    await verifyProducts(page, "#product-5", "Noir Jacket", "60.00");
    await verifyProducts(page, "#product-6", "Striped Top", "50.00");
    await verifyProducts(page, "#product-7", "White Sandals", "25.00");
    await expect(page.locator("#product-7").locator(".sold-out").getByText("Sold Out")).toBeVisible();
  });
  ...
  test("Payment methods are shown in the page", async () => {
    ...
    expect(paymentMethods.getByAltText("we accept Amex"));
    expect(paymentMethods.getByAltText("we accept Visa"));
    expect(paymentMethods.getByAltText("we accept Mastercard"));
  });
